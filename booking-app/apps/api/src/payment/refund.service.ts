    ...
    return await withSerializationRetry(
      () =>
        this.prisma.$transaction(async (tx) => {
          ...
          // Refunds already requested but not yet settled have committed that money. The
          // column only counts SUCCEEDED refunds, and the `FOR UPDATE` above does not help:
          // the first request has committed and released its lock long before the second
          // arrives. Without this, two full-amount requests issued while the first is still
          // PENDING both pass, and both are sent to Stripe.
          ...
          const refundable = Money.fromCents(payment.amountCents, payment.currency)
            .minus(Money.fromCents(payment.refundedAmountCents, payment.currency))
            .minus(Money.fromCents(pending._sum.amountCents ?? 0, payment.currency));
