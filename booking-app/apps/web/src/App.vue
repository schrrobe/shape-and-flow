...
/**
 * The office area brings its own chrome.
 *
 * It is a different product for a different reader: wider, English-only, no language switch
 * and no customer footer. Branching here rather than moving the public chrome into a layout
 * component keeps every existing customer route untouched.
 */
const isOffice = computed(() => route.meta.area === 'office');
...
<template>
  <!-- The office area supplies its own skip link, header and width. -->
  <RouterView v-if="isOffice" />
